import numpy as np
import pytest as pt
from rdds.lib.processpool import ProcessPool, DummyPool
...
def f_return_value(x: int, q: SimpleQueue):
    random_delay()
    q.put(x)
...
def test_processpool():
    ...
    args = [(1, q)] * 100
    pool = ProcessPool(fn=f_return_value, args=args)
...
def test_processpool_worker_data_sizes():
    ...
    for worker_instances in [1, 5, 10]:  # Parallel workers generating data
        for data_size in [1, 1E2, 1E3]:  # Floating point array sizes
            ...
            pool = ProcessPool(fn=f_dynamic_data_size,
                               args=args)
...
def test_capture_worker_error():
    ...
    """
    # GIVEN a worker pool
    pool = ProcessPool(fn=f_erroneous, args=[(None, )])
...
def test_dummypool():
    ...
    q = ProcessPool.get_context().SimpleQueue()
    pool = DummyPool(fn=f_return_value, args=[(1, q)] * 10)
...
def test_dummypool_async():
    ...
    q = ProcessPool.get_context().SimpleQueue()
    args = [(1, q)] * 10
    pool = DummyPool(fn=f_return_value, args=args)
    # WHEN running computations
    completed_tasks_queue = pool.run_async()
    # THEN expect behavior to be identical to real pool
    result_sum = 0
...
def test_dummypool_errors():
    ...
    # GIVEN a pool that's running in main thread'
    q = ProcessPool.get_context().SimpleQueue()
    pool = DummyPool(fn=f_erroneous, args=[(1, q)] * 10)
    # WHEN running computations
