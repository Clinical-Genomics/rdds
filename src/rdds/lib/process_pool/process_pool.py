...
class ProcessPool:
    ...
    def _dispatch(self) -> None:
        ...
        def wait_until_available_worker() -> None:
    ...
    def _collect_stopped_tasks(self):
        ...
        while len(self._completed_tasks) < self._nr_expected_tasks:
            self._running_tasks_lock.acquire()
    ...
    @staticmethod
    def _threading_excepthook(except_hook_args: threading.ExceptHookArgs) -> None:
        """
        :raises Exception: Any exception caught in the thread can be raised
        """
    ...
    def run(self) -> List[Task]:
        ...
        :raises EmptyException: In case task result queue is empty (should not happen)
        """
        # Dispatch jobs
        ...
        results: List[Task] = []
        for _ in range(0, self._nr_expected_tasks):
            results.append(async_results.get(timeout=1))
        if len(results) != self._nr_expected_tasks:
            raise ValueError(f'Missing Task results: {results}, expected {self._nr_expected_tasks} results')
        return results
    ...
    def close(self):
        ...
        :return:
        :raise Exception: In case of currently running processes.
        """
